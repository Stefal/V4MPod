...
# TODO erreur de lecture ou conversion de subsectimeoriginal des photos existantes
#           009720 devient 972000
# TODO : Attention aux cas ou il manque le dernier log en cas de crash. !!!!!!!!!!
# TODO : gérer les cas où on souhaite ne faire le traitement qu'avec une seule caméra.
#            actuellement, le traitement des algo est dépendant du numéro de la caméra. Si je demande
#            à traiter seulement la cam n°3, ça ne marche pas car l'algo attend la liste de fichier n°3 qui
#           n'existe pas puisque les autres cams ont été ignorées. De la même facon, si la cam n° 3 devient la seule
#           à être traitée, et donc devient la n°0, la recherche dans loglist ne correspond à une autre cam.
#           Il faut revoir tout ça pour traiter un ensembre regroupant les fichiers, le log, le pic count, d'une seule caméra.
#           ET/OU utiliser un générateur, mais ça impose d'avoir des listes vides.
...
def correlate_nearest_time(loglist, piclist, user_delta = True):
    ...
    try:
        ...
        for i, log_line in enumerate(loglist[:int(len(loglist) // 20 + 1)]):
            ...
            print("calcul {0} - {1} : {2}".format(log_line.log_timestamp, piclist[i].DateTimeOriginal, (log_line.log_timestamp - piclist[i].DateTimeOriginal).total_seconds()))
    ...
    for i, pic in enumerate(piclist):
        ...
        try:
            ...
            #if pic.path != None:
                #print("A Calcul de la diff entre {0} et {1}".format(loglist[i + gap].log_timestamp, os.path.basename(pic.path)))
                #print("B Calcul de la diff entre {0} et {1}".format(loglist[i + gap + n].log_timestamp, os.path.basename(pic.path)))
                
            while next_delta <= delta:
                #print("="*10)
                #print("delta  = {0} pour loglist {1} et piclist {2}".format(delta, loglist[i+gap].log_timestamp, os.path.basename(pic.path)))
                #print("delta2 = {0} pour loglist {1} et piclist {2}".format(abs((loglist[i + gap + n].log_timestamp - pic.DateTimeOriginal).total_seconds() - avg_delta), loglist[i+gap + n].log_timestamp, os.path.basename(pic.path)))
                delta = next_delta
                n = n + 1
                next_delta = abs(
            ...
            if pic.path is not None:
                print("Association de log {0} avec pic {1}".format(loglist[i + gap + n - 1].log_timestamp, os.path.basename(pic.path)))
        ...
        for missing_pic in range(n - 1):
            ...
            try:
                print("=" * 30)
                print("Il manque une photo pour {0} :".format(loglist[i + gap]))
...
def correlate_nearest_time_exlusive(loglist, piclist, user_delta = True):
    """Try to find the right image for each log's timestamp.
    Find the closest image for each timestamp in the log.
    :param user_delta:
    :param loglist: a list of log_infos nametuple
    ...
    for i, pic in enumerate(piclist):
        ...
        try:
            ...
            if  i + 1 < len(piclist) and piclist[i+1].path is None:
                ...
                #Pour cette raison, on ajout 0.5s pour éviter que la photo soit calé sur le timestamp suivant.
                #Le try except est là pour éviter l'erreur pour la toute première photo.
                try:
                    standby_delay = 0.5 if (pic.DateTimeOriginal - piclist[i-1].DateTimeOriginal).total_seconds > 60 else 0
                except IndexError:
                    print("première photo, elle sera un peu en retard")
                    standby_delay = 0.5
                ...
                print("A Calcul de la diff entre {0} et {1}".format(cp_loglist[i + gap].log_timestamp, short_path))
                print("B Calcul de la diff entre {0} et {1}".format(cp_loglist[i + gap + n].log_timestamp, short_path))
            ...
            while next_delta <= delta:
                ...
                print("="*10)
                print("delta  = {0} pour loglist {1} et piclist {2}".format(delta, cp_loglist[i + gap + n- 1].log_timestamp, os.path.basename(pic.path)))
                print("delta2 = {0} pour loglist {1} et piclist {2}".format(next_delta, cp_loglist[i + gap + n].log_timestamp, os.path.basename(pic.path)))
            ...
            if pic.path is not None:
                print(">>>>Association de log {0} avec pic {1}".format(cp_loglist[i  + gap + n - 1].log_timestamp, os.path.basename(pic.path)))
            """# On recalcule le delta habituel entre le log et les images
            # TODO peut-être renommer la variable avg_delta, qui n'est plus une moyenne.
            # ou bien affecter avg_delta vers une autre variable
            avg_delta = (loglist[i + gap + n - 1].log_timestamp - pic.DateTimeOriginal).total_seconds()
    ...
    piclist_corrected=manual_timestamp(loglist, piclist_corrected)
    deviation = standard_deviation(compute_delta3(loglist, piclist_corrected))
    print("standard deviation : ", deviation)
    
    return piclist_corrected, deviation

def correlate_nearest_time_manual(loglist, piclist, user_delta = True):
    """Try to find the right image for each log's timestamp.
    Find the closest image for each timestamp in the log.
    :param user_delta:
    :param loglist: a list of log_infos nametuple
...
def manual_timestamp(loglist, piclist):
    idx_start = 0
    idx_range = 100
    total_lenght = len(loglist)
    piclist = piclist[:]
    while True:
        ...
        for i, log_line in enumerate(loglist[idx_start:idx_end], idx_start):
            try:
                ...
                print("{0:8} : calcul {1} - {2} : {3}".format(i, log_line.log_timestamp, piclist[i].DateTimeOriginal, (log_line.log_timestamp - piclist[i].DateTimeOriginal).total_seconds()))
            except ZeroDivisionError:
                # print("except")
                delta_list.append((loglist[0].log_timestamp - piclist[0].DateTimeOriginal).total_seconds())
            # print(delta_list)
            except IndexError:
                print("{0:8} : calcul {1}".format(i, log_line.log_timestamp, ))
...
def correlate_manual(loglist, piclist, user_delta = True):
    """Try to find the right image for each log's timestamp.
    Find the closest image for each timestamp in the log.
    :param user_delta:
    :param loglist: a list of log_infos nametuple
...
def correlate_log_and_pic(camera_obj, auto=True):
    ...
    if auto:
        ...
        if pic_count_diff == 0:
            ...
            forward = correlate_double_diff_forward(camera_obj.log_list, single_cam_image_list[:], pic_count_diff, cam)
            backward = correlate_double_diff_backward(camera_obj.log_list, single_cam_image_list[:], pic_count_diff, cam)
            nearest = correlate_nearest_time_exlusive(camera_obj.log_list, single_cam_image_list[:])
            #nearest = correlate_nearest_time_exlusive(loglist[:], image_list[cam][:])
            #nearest = correlate_nearest_time_manual(loglist[:], image_list[cam][:])
            ...
            nearest = correlate_nearest_time_exlusive(camera_obj.log_list, camera_obj.image_list[:], user_delta = True)
            print("Time deviation before correction : ", original_deviation)
            print("=" * 80)
            #print("1 : double diff forward deviation: ", forward[1])
            #print("2 : double diff backward deviation: ", backward[1])
        ...
        #piclist_corrected, deviation = correlate_manual(camera_obj.log_list, nearest, user_delta = True)
        #piclist_corrected, deviation = correlate_manual(camera_obj.log_list, camera_obj.image_list[:], user_delta = True)
        #piclist_corrected, deviation = correlate_manual(camera_obj.log_list, single_cam_image_list, user_delta = True)
        piclist_corrected, deviation = correlate_nearest_time_exlusive(camera_obj.log_list, camera_obj.image_list[:], user_delta = True)
    ...
    if args.compare:
        for cam in cam_group:
            ...
            for result in compare_result:
                print("{0} meters between {1} and {2}".format(result[0], os.path.basename(result[1].path), os.path.basename(result[2].path)))
            print("{} pictures couple have more than {} meters between them".format(len(compare_result), max_distance))
