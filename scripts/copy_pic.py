...
def get_drivelist():
    ...
    """
    if 'win' in sys.platform:
        drivelist = subprocess.Popen('wmic logicaldisk get name,volumename, drivetype, volumeserialnumber /format:CSV', shell=True, stdout=subprocess.PIPE)
...
def start_copy_thread():
    ...
    picQueue4.join()
    picQueue5.join()
    print len(piclist[:pic_end]), "pictures copied in", (datetime.datetime.now() - start_time).total_seconds(), "seconds"
    ...
    while not queue.empty():
        ...
        if not os.path.exists(dest_path):
            try:
                ...
                pass
        picture_name = timestamp.strftime("%Y-%m-%d_%HH%Mmn%Ss") + "-Cam_" + camid + "-" + os.path.basename(picture_path)
        print "copying", picture_path, " to ", os.path.join(dest_path, picture_name)
        shutil.copyfile(picture_path, os.path.join(dest_path, picture_name))  #TODO: add an option to choose overwrite destination, or skip
        queue.task_done()
